...
func main() {
	...
	addr := getenv("USER_SVC_ADDR", ":9001")
	...
	s := grpc.NewServer()
	s.RegisterService(&usersvc.ServiceDesc, usersvc.NewServer())

	quit := make(chan os.Signal, 1)
	signal.Notify(quit, syscall.SIGINT, syscall.SIGTERM)

	go func() {
		log.Info("user-svc listening", zap.String("addr", addr))
		if err := s.Serve(lis); err != nil {
			log.Fatal("user-svc error", zap.Error(err))
		}
	}()
